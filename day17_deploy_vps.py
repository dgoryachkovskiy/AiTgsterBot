...
DEFAULT_USER = "root"
DEFAULT_APP_DIR = "/opt/day17-github-mcp"
DEFAULT_PORT = 8000
SERVICE_NAME = "day17-github-mcp"
REMOTE_REQUIREMENTS = "\n".join(
    [
        "mcp>=1.28,<2",
        "httpx>=0.28,<1",
...
def load_settings() -> dict[str, str | int]:
    load_dotenv()
    return {
        "host": os.getenv("DAY17_VPS_HOST", DEFAULT_HOST),
        "user": os.getenv("DAY17_VPS_USER", DEFAULT_USER),
        "password": require_env("DAY17_VPS_PASSWORD"),
        "app_dir": os.getenv("DAY17_VPS_APP_DIR", DEFAULT_APP_DIR),
        "port": int(os.getenv("DAY17_MCP_PORT", str(DEFAULT_PORT))),
    }
...
def upload_project_files(client, settings: dict[str, str | int]) -> None:
    app_dir = str(settings["app_dir"])
    app_dir_q = shlex.quote(app_dir)
    run(client, f"mkdir -p {app_dir_q}")
    sftp = client.open_sftp()
    try:
        sftp.put(str(Path("day17_github_mcp_server.py")), posixpath.join(app_dir, "day17_github_mcp_server.py"))
...
def service_unit(settings: dict[str, str | int]) -> str:
    ...
    return f"""[Unit]
Description=Day 17 GitHub MCP server
...
ExecStart={app_dir}/.venv/bin/python {app_dir}/day17_github_mcp_server.py serve --host 0.0.0.0 --port {port}
Restart=always
RestartSec=3
...
def install_remote_service(client, settings: dict[str, str | int]) -> None:
    app_dir = str(settings["app_dir"])
    app_dir_q = shlex.quote(app_dir)
...
def deploy() -> None:
    ...
    try:
        upload_project_files(client, settings)
        install_remote_service(client, settings)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy Day 17 MCP server to VPS over SSH.")
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("deploy", help="copy files, install service, start remote MCP server")

...
DEFAULT_USER = "root"
DEFAULT_APP_DIR = "/opt/day19-worldcup-pipeline-mcp"
DEFAULT_PORT = 8004
SERVICE_NAME = "day19-worldcup-pipeline-mcp"
...
def load_settings() -> dict[str, str | int]:
    load_dotenv()
    return {
        "host": os.getenv("DAY19_VPS_HOST") or os.getenv("DAY18_VPS_HOST", DEFAULT_HOST),
        "user": os.getenv("DAY19_VPS_USER") or os.getenv("DAY18_VPS_USER", DEFAULT_USER),
        "password": os.getenv("DAY19_VPS_PASSWORD") or os.getenv("DAY18_VPS_PASSWORD") or "",
        "app_dir": os.getenv("DAY19_VPS_APP_DIR", DEFAULT_APP_DIR),
        "port": int(os.getenv("DAY19_MCP_PORT", str(DEFAULT_PORT))),
    }
...
def upload_files(client, settings: dict[str, str | int]) -> None:
    ...
    try:
        sftp.put(str(Path("day19_worldcup_pipeline_mcp_server.py")), posixpath.join(app_dir, "day19_worldcup_pipeline_mcp_server.py"))
...
def service_unit(settings: dict[str, str | int]) -> str:
    ...
    return f"""[Unit]
Description=Day 19 World Cup MCP tool composition pipeline
...
ExecStart={app_dir}/.venv/bin/python {app_dir}/day19_worldcup_pipeline_mcp_server.py serve --host 0.0.0.0 --port {port}
Restart=always
RestartSec=3
Environment=PYTHONUNBUFFERED=1
Environment=DAY19_STORE_DIR={store_dir}
...
def verify_remote_http(client, settings: dict[str, str | int]) -> None:
    port = int(settings["port"])
    run(client, f"curl -i --max-time 10 http://127.0.0.1:{port}/mcp | head -n 20")


def verify_remote_mcp(settings: dict[str, str | int]) -> None:
    url = f"http://{settings['host']}:{settings['port']}/mcp"
    subprocess.run([sys.executable, "day19_pipeline_agent.py", "tools", "--url", url], check=True)
...
def deploy() -> None:
    ...
    try:
        ...
        print(f"auth_error={type(error).__name__}: {error}")
        print("set DAY19_VPS_PASSWORD in .env or configure an SSH key, then run deploy again")
        raise SystemExit(1)
    try:
        upload_files(client, settings)
        install_service(client, settings)
        verify_remote_http(client, settings)
    finally:
        client.close()
    verify_remote_mcp(settings)
    print(f"remote_mcp_url=http://{settings['host']}:{settings['port']}/mcp")
    print("deploy_done=True")


def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy Day 19 World Cup MCP pipeline server to VPS.")
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("deploy", help="copy files, install systemd service, start MCP pipeline server")
